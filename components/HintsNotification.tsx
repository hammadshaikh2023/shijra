  ...
  useEffect(() => {
    const fetchHints = async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/hints?individualId=${targetId}`);
        const result = await res.json();
        
        if (result.success) {
           setHints(result.data);
           setHasNew(result.data.length > 0);
        }
      } catch (err) {
        console.error("Vercel API Match Error:", err);
      } finally {
        setLoading(false);
      }
    };
          ...
          <div className="bg-navy-900/90 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl overflow-hidden relative">
            ...
            <div className="p-4 border-b border-white/5 flex justify-between items-center bg-navy-950/50">
               <div>
                 ...
                 <p className="text-[10px] text-slate-400">Vercel Edge Optimized</p>
               </div>
            ...
            <div className="max-h-[400px] overflow-y-auto p-2 scrollbar-thin scrollbar-thumb-white/10">
              {loading ? (
                <div className="py-8 text-center flex flex-col items-center">
                   ...
                   <span className="text-xs text-slate-500 animate-pulse">Syncing matches...</span>
                </div>
              ) : (
            ...
            <div className="p-3 bg-navy-950/80 border-t border-white/5 text-center">
               <span className="text-[8px] text-slate-600 font-black uppercase tracking-[0.3em]">Sanctuary Node: Vercel Edge</span>
            </div>
          </div>
        </div>
      )}
